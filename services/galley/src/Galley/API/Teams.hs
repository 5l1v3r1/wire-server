...
addTeamMemberInternal tid origin originConn newMem mems = do
  ...
  push1 $ newPush1 (Local (new ^. userId)) (TeamEvent e) (r origin new) & pushConn .~ originConn
  where
...
finishCreateTeam team owner others zcon = do
  ...
  push1 $ newPush1 (Local zusr) (TeamEvent e) (list1 (userRecipient zusr) r) & pushConn .~ zcon
  pure (team ^. teamId)
